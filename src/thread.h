...
typedef struct _thread_ thread_t;
...
thread_t* NanoVM_create_thread (nanovm_t* vm);
      int NanoVM_stop_thread   (thread_t* thread); // only mark `stop`
     void NanoVM_release_thread(thread_t* thread);
